...
void Robot::application_compteur_balle::traitement_camera()
{
    ...
    if( StatusCheck::m_cur_mode == READY || StatusCheck::m_cur_mode == VISION || StatusCheck::m_cur_mode == SOCCER || StatusCheck::m_cur_mode == MOTION )
    {
        ...
        supprimerPixel(0,0);
        for(int j=0;j< m_ressources->m_NumberOfPixels;i++)
        {
            int ligne =i / WIDTH;
            int colone =i % WIDTH;
            if (m_tab_pixel_drapeau == false)
            {
                ...
                if(m_ressources.get_black_finder()->m_result->m_ImageData[i] == 1)
                {
                    r = 0;
                    g = 0;
                    b = 0;
                ...
                if(m_ressources.get_green_finder()->m_result->m_ImageData[i] == 1)
                {
                    r = 0;
                    g = 106;
                    b = 78;
