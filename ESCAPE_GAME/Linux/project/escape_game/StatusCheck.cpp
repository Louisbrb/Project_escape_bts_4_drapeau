...
void StatusCheck::Check(CM730 &cm730)
{
    ...
    if(m_old_btn & BTN_MODE)
    {
        ...
        else if(m_cur_mode == SOCCER)
        {
            ...
            LinuxActionScript::PlayMP3("/darwin/Data/mp3/Autonomous soccer mode.mp3");
        }
        else if(m_cur_mode == MOTION)
        {
            cm730.WriteByte(CM730::P_LED_PANNEL, 0x02, NULL);
            LinuxActionScript::PlayMP3("/darwin/Data/mp3/Interactive motion mode.mp3");
        }
        else if(m_cur_mode == VISION)
        {
            cm730.WriteByte(CM730::P_LED_PANNEL, 0x04, NULL);
            LinuxActionScript::PlayMP3("/darwin/Data/mp3/Vision processing mode.mp3");
        }
    }
    ...
    if(m_old_btn & BTN_START)
    {
        if(m_is_started == 0)
        {
            ...
            if(m_cur_mode == MOTION || m_cur_mode == VISION || m_cur_mode == SOCCER )
            {
